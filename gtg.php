...
// Check if user has mb parameter
$hasMb = false;
$mbParam = '';

// Try to get mb from $_GET first
if (isset($_GET['mb'])) {
  $mbParam = $_GET['mb'];
  $hasMb = trim($mbParam) !== '';
} else {
  ...
  if ($queryString) {
    parse_str($queryString, $parsedParams);
    if (isset($parsedParams['mb'])) {
      $mbParam = $parsedParams['mb'];
      $hasMb = trim($mbParam) !== '';
    }
  }

  // If still no mb, try to extract from referrer URL
  if (!$hasMb && $currentReferrer) {
    $referrerUrl = parse_url($currentReferrer);
    if (isset($referrerUrl['query'])) {
      parse_str($referrerUrl['query'], $referrerParams);
      if (isset($referrerParams['mb'])) {
        $mbParam = $referrerParams['mb'];
        $hasMb = trim($mbParam) !== '';
      }
    }
  }
...
// Logic:
// gtg=1 if user came from spy referrer OR doesn't have "key"
// If they pass all tests, don't send gtg to Ringba
